	...
	({ offerTypes, uk }) => {
		...
		if (submitting === 'success') {
			return (
				<div className="p-2 rounded-lg bg-indigo-600 shadow-lg sm:p-3 text-center text-lg">
					<p className="mx-3 font-medium text-white">{uk ? 'Надіслано, незабаром ми зв’яжемося з вами.' : 'Odesláno, brzy se vám ozveme.'}</p>
				</div>
			)
		}
		...
		return (
			<form className="grid grid-cols-1 gap-y-6 sm:gap-x-8" onSubmit={submit}>
				<div>
					{submitting === 'error' && <p>{uk ? 'Вибачте, щось пішло не так. Будь ласка спробуйте ще раз.' : 'Omlouvám se, něco se pokazilo. Zkuste to prosím znovu.'}</p>}
				</div>
				<div>
						...
						<div key={offerType.id} className="mt-1">
							<div>
								<label>
									...
									/>
									<span className="pl-2 text-sm font-medium text-gray-700">{uk ? offerType.nameUK : offerType.name}</span>
								</label>
							</div>
						</div>
					))}
					<div>
						<label>
							<input
								...
								type="checkbox"
							/>
							<span className="pl-2 text-sm font-medium text-gray-700">{uk ? 'Iншая потреба' : 'Jiná potřeba'}</span>
						</label>
						{otherCheckedState && (
							<textarea
								disabled={disabled}
								value={state.otherType}
								onChange={(e) => setState({ ...state, otherType: e.target.value })}
								className="py-3 px-4 block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 border border-gray-300 rounded-md"
							/>
						)}
					</div>
				...
				<div>
					{errors.length > 0 && <p className="text-center">{uk ? 'Переконайтеся, що ви все завершили правильно.' : 'Zkontrolujte, zda jste vše vyplnili správněuk.'}</p>}
				</div>
			</form>
		)
	}
